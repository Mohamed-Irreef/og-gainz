...
const MEALTYPE_ADMIN_PROJECTION = {
	name: 1,
	slug: 1,
	description: 1,
	displayOrder: 1,
...
const adminListMealTypes = async (req, res, next) => {
	try {
		...
		const [items, total] = await Promise.all([
			MealType.find(filter)
				.select(MEALTYPE_ADMIN_PROJECTION)
				.sort({ displayOrder: 1, createdAt: -1 })
				.skip(skip)
				.limit(limit)
				.lean(),
			MealType.countDocuments(filter),
		]);
...
const adminCreateMealType = async (req, res, next) => {
	try {
		const payload = { ...req.body };
		if (!payload.slug && payload.name) {
			payload.slug = toSlug(payload.name);
		}
		const created = await MealType.create(payload);
...
const adminUpdateMealType = async (req, res, next) => {
	try {
		const { id } = req.params;
		const payload = { ...req.body };
		if (payload.slug) payload.slug = String(payload.slug).trim().toLowerCase();
		const updated = await MealType.findByIdAndUpdate(id, payload, {
			new: true,
			runValidators: true,
		});
		if (!updated) {
			return res.status(404).json({ status: 'error', message: 'Meal type not found' });
		}
...
const adminDeleteMealType = async (req, res, next) => {
	try {
		const { id } = req.params;
